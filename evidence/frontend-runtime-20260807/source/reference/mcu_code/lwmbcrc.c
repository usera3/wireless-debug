...
// CRC16高位字节查表
static const uint8_t crc16_hi_table[] =
{
...
// CRC16低位字节查表
static const uint8_t crc16_lo_table[] =
{
...
uint16_t lwmb_crc16(uint8_t *data, uint16_t len)
{
    ...
    while(len--) {
        index = crc_lo ^ *(data++);
        crc_lo = crc_hi ^ crc16_hi_table[index];
        crc_hi = crc16_lo_table[index];
    }
...
{
    uint8_t crc_hi = (current >> 8) & 0xFF;
    uint8_t crc_lo = current & 0xFF;
    uint16_t index;
